...
import (
	...
	"emperror.dev/errors"
	"github.com/cisco-open/libnasp/components/kafka-protocol-go/pkg/protocol/messages/describeuserscramcredentials/request"
...
var requestUsers = fields.Context{
	SpecName:                        "Users",
...
func (o *Request) Users() []request.UserName {
	return o.users
}

func (o *Request) SetUsers(val []request.UserName) {
	o.isNil = false
	o.users = val
}
...
func (o *Request) Read(buf *bytes.Reader, version int16) error {
	...
	usersField := fields.ArrayOfStruct[request.UserName, *request.UserName]{Context: requestUsers}
	users, err := usersField.Read(buf, version)
	if err != nil {
		return errors.WrapIf(err, "couldn't set \"users\" field")
	}
	o.users = users
...
func (o *Request) Write(buf *typesbytes.SliceWriter, version int16) error {
	...
	usersField := fields.ArrayOfStruct[request.UserName, *request.UserName]{Context: requestUsers}
	if err := usersField.Write(buf, version, o.users); err != nil {
		return errors.WrapIf(err, "couldn't serialize \"users\" field")
	}
...
func (o *Request) MarshalJSON() ([]byte, error) {
	...
	s := make([][]byte, 0, 2)
	if b, err := fields.ArrayOfStructMarshalJSON("users", o.users); err != nil {
		return nil, err
	} else {
		s = append(s, b)
	}
...
func (o *Request) Equal(that *Request) bool {
	...
	if len(o.users) != len(that.users) {
		return false
	}
	for i := range o.users {
		if !o.users[i].Equal(&that.users[i]) {
			return false
		}
	}
...
func (o *Request) SizeInBytes(version int16) (int, error) {
	...
	usersField := fields.ArrayOfStruct[request.UserName, *request.UserName]{Context: requestUsers}
	fieldSize, err = usersField.SizeInBytes(version, o.users)
	if err != nil {
		return 0, errors.WrapIf(err, "couldn't compute size of \"users\" field")
	}
	size += fieldSize
