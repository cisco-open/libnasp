...
import (
	...
	"emperror.dev/errors"
	"github.com/cisco-open/libnasp/components/kafka-protocol-go/pkg/protocol/messages/describedelegationtoken/response"
...
var responseTokens = fields.Context{
	SpecName:                    "Tokens",
	LowestSupportedVersion:      0,
	HighestSupportedVersion:     32767,
	LowestSupportedFlexVersion:  2,
...
type Response struct {
	tokens              []response.DescribedDelegationToken
...
func (o *Response) Tokens() []response.DescribedDelegationToken {
	return o.tokens
}

func (o *Response) SetTokens(val []response.DescribedDelegationToken) {
	o.isNil = false
	o.tokens = val
}
...
func (o *Response) Read(buf *bytes.Reader, version int16) error {
	...
	tokensField := fields.ArrayOfStruct[response.DescribedDelegationToken, *response.DescribedDelegationToken]{Context: responseTokens}
	tokens, err := tokensField.Read(buf, version)
	if err != nil {
		return errors.WrapIf(err, "couldn't set \"tokens\" field")
	}
	o.tokens = tokens
...
func (o *Response) Write(buf *typesbytes.SliceWriter, version int16) error {
	...
	tokensField := fields.ArrayOfStruct[response.DescribedDelegationToken, *response.DescribedDelegationToken]{Context: responseTokens}
	if err := tokensField.Write(buf, version, o.tokens); err != nil {
		return errors.WrapIf(err, "couldn't serialize \"tokens\" field")
	}
...
func (o *Response) MarshalJSON() ([]byte, error) {
	...
	if b, err := fields.ArrayOfStructMarshalJSON("tokens", o.tokens); err != nil {
		return nil, err
	} else {
		s = append(s, b)
	}
...
func (o *Response) Equal(that *Response) bool {
	...
	if len(o.tokens) != len(that.tokens) {
		return false
	}
	for i := range o.tokens {
		if !o.tokens[i].Equal(&that.tokens[i]) {
			return false
		}
	}
...
func (o *Response) SizeInBytes(version int16) (int, error) {
	...
	tokensField := fields.ArrayOfStruct[response.DescribedDelegationToken, *response.DescribedDelegationToken]{Context: responseTokens}
	fieldSize, err = tokensField.SizeInBytes(version, o.tokens)
	if err != nil {
		return 0, errors.WrapIf(err, "couldn't compute size of \"tokens\" field")
	}
	size += fieldSize
