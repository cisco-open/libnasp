// Code generated by kafka-protocol-go. DO NOT EDIT.

// Copyright (c) 2023 Cisco and/or its affiliates. All rights reserved.
//
//	Licensed under the Apache License, Version 2.0 (the "License");
//	you may not use this file except in compliance with the License.
//	You may obtain a copy of the License at
//
//	     https://www.apache.org/licenses/LICENSE-2.0
//
//	Unless required by applicable law or agreed to in writing, software
//	distributed under the License is distributed on an "AS IS" BASIS,
//	WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
//	See the License for the specific language governing permissions and
//	limitations under the License.
package response

import (
	"bytes"
	"strconv"
	"strings"

	"emperror.dev/errors"
	typesbytes "github.com/cisco-open/libnasp/components/kafka-protocol-go/pkg/protocol/types/bytes"
	"github.com/cisco-open/libnasp/components/kafka-protocol-go/pkg/protocol/types/fields"
	"github.com/cisco-open/libnasp/components/kafka-protocol-go/pkg/protocol/types/varint"
)

var coordinatorKey = fields.Context{
	SpecName:                    "Key",
	LowestSupportedVersion:      4,
	HighestSupportedVersion:     32767,
	LowestSupportedFlexVersion:  3,
	HighestSupportedFlexVersion: 32767,
}
var coordinatorNodeId = fields.Context{
	SpecName:                    "NodeId",
	LowestSupportedVersion:      4,
	HighestSupportedVersion:     32767,
	LowestSupportedFlexVersion:  3,
	HighestSupportedFlexVersion: 32767,
}
var coordinatorHost = fields.Context{
	SpecName:                    "Host",
	LowestSupportedVersion:      4,
	HighestSupportedVersion:     32767,
	LowestSupportedFlexVersion:  3,
	HighestSupportedFlexVersion: 32767,
}
var coordinatorPort = fields.Context{
	SpecName:                    "Port",
	LowestSupportedVersion:      4,
	HighestSupportedVersion:     32767,
	LowestSupportedFlexVersion:  3,
	HighestSupportedFlexVersion: 32767,
}
var coordinatorErrorCode = fields.Context{
	SpecName:                    "ErrorCode",
	LowestSupportedVersion:      4,
	HighestSupportedVersion:     32767,
	LowestSupportedFlexVersion:  3,
	HighestSupportedFlexVersion: 32767,
}
var coordinatorErrorMessage = fields.Context{
	SpecName:                        "ErrorMessage",
	LowestSupportedVersion:          4,
	HighestSupportedVersion:         32767,
	LowestSupportedFlexVersion:      3,
	HighestSupportedFlexVersion:     32767,
	LowestSupportedNullableVersion:  4,
	HighestSupportedNullableVersion: 32767,
}

type Coordinator struct {
	unknownTaggedFields []fields.RawTaggedField
	key                 fields.NullableString
	host                fields.NullableString
	errorMessage        fields.NullableString
	nodeId              int32
	port                int32
	errorCode           int16
	isNil               bool
}

func (o *Coordinator) Key() fields.NullableString {
	return o.key
}

func (o *Coordinator) SetKey(val fields.NullableString) {
	o.isNil = false
	o.key = val
}

func (o *Coordinator) NodeId() int32 {
	return o.nodeId
}

func (o *Coordinator) SetNodeId(val int32) {
	o.isNil = false
	o.nodeId = val
}

func (o *Coordinator) Host() fields.NullableString {
	return o.host
}

func (o *Coordinator) SetHost(val fields.NullableString) {
	o.isNil = false
	o.host = val
}

func (o *Coordinator) Port() int32 {
	return o.port
}

func (o *Coordinator) SetPort(val int32) {
	o.isNil = false
	o.port = val
}

func (o *Coordinator) ErrorCode() int16 {
	return o.errorCode
}

func (o *Coordinator) SetErrorCode(val int16) {
	o.isNil = false
	o.errorCode = val
}

func (o *Coordinator) ErrorMessage() fields.NullableString {
	return o.errorMessage
}

func (o *Coordinator) SetErrorMessage(val fields.NullableString) {
	o.isNil = false
	o.errorMessage = val
}

func (o *Coordinator) UnknownTaggedFields() []fields.RawTaggedField {
	return o.unknownTaggedFields
}

func (o *Coordinator) SetUnknownTaggedFields(val []fields.RawTaggedField) {
	o.unknownTaggedFields = val
}

func (o *Coordinator) Read(buf *bytes.Reader, version int16) error {
	o.SetDefault()

	keyField := fields.String{Context: coordinatorKey}
	if err := keyField.Read(buf, version, &o.key); err != nil {
		return errors.WrapIf(err, "couldn't set \"key\" field")
	}

	nodeIdField := fields.Int32{Context: coordinatorNodeId}
	if err := nodeIdField.Read(buf, version, &o.nodeId); err != nil {
		return errors.WrapIf(err, "couldn't set \"nodeId\" field")
	}

	hostField := fields.String{Context: coordinatorHost}
	if err := hostField.Read(buf, version, &o.host); err != nil {
		return errors.WrapIf(err, "couldn't set \"host\" field")
	}

	portField := fields.Int32{Context: coordinatorPort}
	if err := portField.Read(buf, version, &o.port); err != nil {
		return errors.WrapIf(err, "couldn't set \"port\" field")
	}

	errorCodeField := fields.Int16{Context: coordinatorErrorCode}
	if err := errorCodeField.Read(buf, version, &o.errorCode); err != nil {
		return errors.WrapIf(err, "couldn't set \"errorCode\" field")
	}

	errorMessageField := fields.String{Context: coordinatorErrorMessage}
	if err := errorMessageField.Read(buf, version, &o.errorMessage); err != nil {
		return errors.WrapIf(err, "couldn't set \"errorMessage\" field")
	}

	// process tagged fields

	if version < CoordinatorLowestSupportedFlexVersion() || version > CoordinatorHighestSupportedFlexVersion() {
		// tagged fields are only supported by flexible versions
		o.isNil = false
		return nil
	}

	if buf.Len() == 0 {
		o.isNil = false
		return nil
	}

	rawTaggedFields, err := fields.ReadRawTaggedFields(buf)
	if err != nil {
		return err
	}

	o.unknownTaggedFields = rawTaggedFields

	o.isNil = false
	return nil
}

func (o *Coordinator) Write(buf *typesbytes.SliceWriter, version int16) error {
	if o.IsNil() {
		return nil
	}
	if err := o.validateNonIgnorableFields(version); err != nil {
		return err
	}

	keyField := fields.String{Context: coordinatorKey}
	if err := keyField.Write(buf, version, o.key); err != nil {
		return errors.WrapIf(err, "couldn't serialize \"key\" field")
	}
	nodeIdField := fields.Int32{Context: coordinatorNodeId}
	if err := nodeIdField.Write(buf, version, o.nodeId); err != nil {
		return errors.WrapIf(err, "couldn't serialize \"nodeId\" field")
	}
	hostField := fields.String{Context: coordinatorHost}
	if err := hostField.Write(buf, version, o.host); err != nil {
		return errors.WrapIf(err, "couldn't serialize \"host\" field")
	}
	portField := fields.Int32{Context: coordinatorPort}
	if err := portField.Write(buf, version, o.port); err != nil {
		return errors.WrapIf(err, "couldn't serialize \"port\" field")
	}
	errorCodeField := fields.Int16{Context: coordinatorErrorCode}
	if err := errorCodeField.Write(buf, version, o.errorCode); err != nil {
		return errors.WrapIf(err, "couldn't serialize \"errorCode\" field")
	}
	errorMessageField := fields.String{Context: coordinatorErrorMessage}
	if err := errorMessageField.Write(buf, version, o.errorMessage); err != nil {
		return errors.WrapIf(err, "couldn't serialize \"errorMessage\" field")
	}

	// serialize tagged fields
	numTaggedFields := o.getTaggedFieldsCount(version)
	if version < CoordinatorLowestSupportedFlexVersion() || version > CoordinatorHighestSupportedFlexVersion() {
		if numTaggedFields > 0 {
			return errors.New(strings.Join([]string{"tagged fields were set, but version", strconv.Itoa(int(version)), "of this message does not support them"}, " "))
		}

		return nil
	}

	rawTaggedFields := make([]fields.RawTaggedField, 0, numTaggedFields)
	rawTaggedFields = append(rawTaggedFields, o.unknownTaggedFields...)

	if err := fields.WriteRawTaggedFields(buf, rawTaggedFields); err != nil {
		return errors.WrapIf(err, "couldn't serialize tagged fields")
	}

	return nil
}

func (o *Coordinator) String() string {
	s, err := o.MarshalJSON()
	if err != nil {
		return err.Error()
	}

	return string(s)
}

func (o *Coordinator) MarshalJSON() ([]byte, error) {
	if o == nil || o.IsNil() {
		return []byte("null"), nil
	}

	s := make([][]byte, 0, 7)
	if b, err := fields.MarshalPrimitiveTypeJSON(o.key); err != nil {
		return nil, err
	} else {
		s = append(s, bytes.Join([][]byte{[]byte("\"key\""), b}, []byte(": ")))
	}
	if b, err := fields.MarshalPrimitiveTypeJSON(o.nodeId); err != nil {
		return nil, err
	} else {
		s = append(s, bytes.Join([][]byte{[]byte("\"nodeId\""), b}, []byte(": ")))
	}
	if b, err := fields.MarshalPrimitiveTypeJSON(o.host); err != nil {
		return nil, err
	} else {
		s = append(s, bytes.Join([][]byte{[]byte("\"host\""), b}, []byte(": ")))
	}
	if b, err := fields.MarshalPrimitiveTypeJSON(o.port); err != nil {
		return nil, err
	} else {
		s = append(s, bytes.Join([][]byte{[]byte("\"port\""), b}, []byte(": ")))
	}
	if b, err := fields.MarshalPrimitiveTypeJSON(o.errorCode); err != nil {
		return nil, err
	} else {
		s = append(s, bytes.Join([][]byte{[]byte("\"errorCode\""), b}, []byte(": ")))
	}
	if b, err := fields.MarshalPrimitiveTypeJSON(o.errorMessage); err != nil {
		return nil, err
	} else {
		s = append(s, bytes.Join([][]byte{[]byte("\"errorMessage\""), b}, []byte(": ")))
	}

	if b, err := fields.ArrayOfStructMarshalJSON("unknownTaggedFields", o.unknownTaggedFields); err != nil {
		return nil, err
	} else {
		s = append(s, b)
	}

	var b bytes.Buffer
	if err := b.WriteByte('{'); err != nil {
		return nil, err
	}
	if _, err := b.Write(bytes.Join(s, []byte(", "))); err != nil {
		return nil, err
	}
	if err := b.WriteByte('}'); err != nil {
		return nil, err
	}

	return b.Bytes(), nil
}

func (o *Coordinator) IsNil() bool {
	return o.isNil
}

func (o *Coordinator) Clear() {
	o.Release()
	o.isNil = true

	o.unknownTaggedFields = nil
}

func (o *Coordinator) SetDefault() {
	for i := range o.unknownTaggedFields {
		o.unknownTaggedFields[i].Release()
	}
	o.unknownTaggedFields = nil
	o.key.SetValue("")
	o.nodeId = 0
	o.host.SetValue("")
	o.port = 0
	o.errorCode = 0
	o.errorMessage.SetValue("")

	o.isNil = false
}

func (o *Coordinator) Equal(that *Coordinator) bool {
	if !fields.RawTaggedFieldsEqual(o.unknownTaggedFields, that.unknownTaggedFields) {
		return false
	}

	if !o.key.Equal(&that.key) {
		return false
	}
	if o.nodeId != that.nodeId {
		return false
	}
	if !o.host.Equal(&that.host) {
		return false
	}
	if o.port != that.port {
		return false
	}
	if o.errorCode != that.errorCode {
		return false
	}
	if !o.errorMessage.Equal(&that.errorMessage) {
		return false
	}

	return true
}

// SizeInBytes returns the size of this data structure in bytes when it's serialized
func (o *Coordinator) SizeInBytes(version int16) (int, error) {
	if o.IsNil() {
		return 0, nil
	}

	if err := o.validateNonIgnorableFields(version); err != nil {
		return 0, err
	}

	size := 0
	fieldSize := 0
	var err error

	keyField := fields.String{Context: coordinatorKey}
	fieldSize, err = keyField.SizeInBytes(version, o.key)
	if err != nil {
		return 0, errors.WrapIf(err, "couldn't compute size of \"key\" field")
	}
	size += fieldSize

	nodeIdField := fields.Int32{Context: coordinatorNodeId}
	fieldSize, err = nodeIdField.SizeInBytes(version, o.nodeId)
	if err != nil {
		return 0, errors.WrapIf(err, "couldn't compute size of \"nodeId\" field")
	}
	size += fieldSize

	hostField := fields.String{Context: coordinatorHost}
	fieldSize, err = hostField.SizeInBytes(version, o.host)
	if err != nil {
		return 0, errors.WrapIf(err, "couldn't compute size of \"host\" field")
	}
	size += fieldSize

	portField := fields.Int32{Context: coordinatorPort}
	fieldSize, err = portField.SizeInBytes(version, o.port)
	if err != nil {
		return 0, errors.WrapIf(err, "couldn't compute size of \"port\" field")
	}
	size += fieldSize

	errorCodeField := fields.Int16{Context: coordinatorErrorCode}
	fieldSize, err = errorCodeField.SizeInBytes(version, o.errorCode)
	if err != nil {
		return 0, errors.WrapIf(err, "couldn't compute size of \"errorCode\" field")
	}
	size += fieldSize

	errorMessageField := fields.String{Context: coordinatorErrorMessage}
	fieldSize, err = errorMessageField.SizeInBytes(version, o.errorMessage)
	if err != nil {
		return 0, errors.WrapIf(err, "couldn't compute size of \"errorMessage\" field")
	}
	size += fieldSize

	// tagged fields
	numTaggedFields := int64(o.getTaggedFieldsCount(version))
	if numTaggedFields > 0xffffffff {
		return 0, errors.New(strings.Join([]string{"invalid tagged fields count:", strconv.Itoa(int(numTaggedFields))}, " "))
	}
	if version < CoordinatorLowestSupportedFlexVersion() || version > CoordinatorHighestSupportedFlexVersion() {
		if numTaggedFields > 0 {
			return 0, errors.New(strings.Join([]string{"tagged fields were set, but version", strconv.Itoa(int(version)), "of this message does not support them"}, " "))
		}

		return size, nil
	}

	taggedFieldsSize := varint.Uint32Size(uint32(numTaggedFields)) // bytes for serializing the number of tagged fields

	for i := range o.unknownTaggedFields {
		length := len(o.unknownTaggedFields[i].Value())
		if int64(length) > 0xffffffff {
			return 0, errors.New(strings.Join([]string{"invalid field value length:", strconv.Itoa(length), ", tag:", strconv.Itoa(int(o.unknownTaggedFields[i].Tag()))}, " "))
		}
		taggedFieldsSize += varint.Uint32Size(o.unknownTaggedFields[i].Tag()) // bytes for serializing the tag of the unknown tag
		taggedFieldsSize += varint.Uint32Size(uint32(length))                 // bytes for serializing the length of the unknown tagged field
		taggedFieldsSize += length
	}

	size += taggedFieldsSize

	return size, nil
}

// Release releases the dynamically allocated fields of this object by returning then to object pools
func (o *Coordinator) Release() {
	if o.IsNil() {
		return
	}

	for i := range o.unknownTaggedFields {
		o.unknownTaggedFields[i].Release()
	}
	o.unknownTaggedFields = nil

	o.key.Release()
	o.host.Release()
	o.errorMessage.Release()
}

func (o *Coordinator) getTaggedFieldsCount(version int16) int {
	numTaggedFields := len(o.unknownTaggedFields)

	return numTaggedFields
}

// validateNonIgnorableFields throws an error if any non-ignorable field not supported by current version is set to
// non-default value
func (o *Coordinator) validateNonIgnorableFields(version int16) error {
	if !coordinatorKey.IsSupportedVersion(version) {
		if o.key.Bytes() != nil {
			return errors.New(strings.Join([]string{"attempted to write non-default \"key\" at version", strconv.Itoa(int(version))}, " "))
		}
	}
	if !coordinatorNodeId.IsSupportedVersion(version) {
		if o.nodeId != 0 {
			return errors.New(strings.Join([]string{"attempted to write non-default \"nodeId\" at version", strconv.Itoa(int(version))}, " "))
		}
	}
	if !coordinatorHost.IsSupportedVersion(version) {
		if o.host.Bytes() != nil {
			return errors.New(strings.Join([]string{"attempted to write non-default \"host\" at version", strconv.Itoa(int(version))}, " "))
		}
	}
	if !coordinatorPort.IsSupportedVersion(version) {
		if o.port != 0 {
			return errors.New(strings.Join([]string{"attempted to write non-default \"port\" at version", strconv.Itoa(int(version))}, " "))
		}
	}
	if !coordinatorErrorCode.IsSupportedVersion(version) {
		if o.errorCode != 0 {
			return errors.New(strings.Join([]string{"attempted to write non-default \"errorCode\" at version", strconv.Itoa(int(version))}, " "))
		}
	}
	return nil
}

func CoordinatorLowestSupportedVersion() int16 {
	return 4
}

func CoordinatorHighestSupportedVersion() int16 {
	return 32767
}

func CoordinatorLowestSupportedFlexVersion() int16 {
	return 3
}

func CoordinatorHighestSupportedFlexVersion() int16 {
	return 32767
}

func CoordinatorDefault() Coordinator {
	var d Coordinator
	d.SetDefault()

	return d
}
